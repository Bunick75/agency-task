...
<!-- ============ for filtering through genre======== -->
    <?php 
	$genreQ = "SELECT DISTINCT genre from movie_details";
	$results = ($conn->query($genreQ)); ?>
...
<script>  
 $(document).ready(function(){  
      ...
      $('#genre').change(function(){  //onchanging the value in the dropdown
           var genre = $(this).val(); // it will take the value of it 
           $.ajax({  
                url:"filter.php", //goes to this url
                method:"POST",  // with post request
                data:{genre:genre},  // along with lang data
                success:function(data){  //on successfully performing the desired action
                     $('#show_movies').html(data);  //displays the dataa in the table whose id is show_movies 
                }  
           });  
